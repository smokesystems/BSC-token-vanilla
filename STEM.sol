...
contract STEM is BEP20, Pausable { 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if ( paused ) {
		    require(operatorsList[sender], "STEM: sender not whitelisted to transfer when paused");
		}
		if ( !allowLiquidityAdding && liquidityOperatorsList[recipient]) {
		    require(liquidityProvidersList[sender], "STEM: sender not whitelisted to add liquidity");
		}
    ...
    function setTaxLiquidityPercent(uint256 newTaxPercent) external onlyOwner {
		require((taxOperationalPercent + newTaxPercent) <= 1200, 'STEM: sum of taxes can`t be more than 1200 (12%)');
	...
	function toggleLiquidityOperatorsList(address account) external onlyOwner returns (bool) {
		require(account != address(pancakeSwapV2Router), 'STEM: pancakeSwapV2Router can`t be removed from list');
